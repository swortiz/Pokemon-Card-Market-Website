...
import { loadCart, removeFromCart } from "./store/cart-store.js";
...
export class CartPage extends LitElement {
  ...
  constructor() {
    ...
    // Load initial cart
    loadCart().then((cart: CartItem[]) => (this.items = cart));

    // Listen for cart updates
    pubsub.subscribe("cart-updated", (cart: CartItem[]) => {
      this.items = [...cart];
    });
  }
    ...
    return html`
      <section class="wrapper">
                  ...
                  (item) => html`
                    <div class="cart-item">
                      ...
                      <div class="info">
                        ...
                        <button class="remove-btn" @click=${() => removeFromCart(item.name)}>
                          Remove
                        </button>
                      </div>
                    </div>
        ...
        <div class="back-btn-wrapper">
          <button class="button-primary" @click=${() => (window.location.href = "/")}>
            ← Back to Home
          </button>
        </div>
